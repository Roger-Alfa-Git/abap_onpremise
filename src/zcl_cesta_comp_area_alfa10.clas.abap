class ZCL_CESTA_COMP_AREA_ALFA10 definition
  public
...
public section.
  ...
  constants AREA_NAME type SHM_AREA_NAME value 'ZCL_CESTA_COMP_AREA_ALFA10' ##NO_TEXT.
  data ROOT type ref to ZCL_CESTA_COMPRAS_ALFA10 read-only .
  ...
  class-methods ATTACH_FOR_READ
    ...
    returning
      value(HANDLE) type ref to ZCL_CESTA_COMP_AREA_ALFA10
    raising
      CX_SHM_INCONSISTENT
  ...
  class-methods INVALIDATE_AREA
    ...
    raising
      CX_SHM_PARAMETER_ERROR .
  class-methods PROPAGATE_INSTANCE
    importing
      !CLIENT type SHM_CLIENT optional
      !INST_NAME type SHM_INST_NAME default CL_SHM_AREA=>DEFAULT_INSTANCE
  ...
  methods SET_ROOT
    importing
      !ROOT type ref to ZCL_CESTA_COMPRAS_ALFA10
    raising
...
private section.
  ...
  constants _TRANSACTIONAL type ABAP_BOOL value ABAP_TRUE ##NO_TEXT.
...
CLASS ZCL_CESTA_COMP_AREA_ALFA10 IMPLEMENTATION.
...
* <
  ...
  IF _trace_active = abap_true.
    IF _trace_service->variant-propagate_area = abap_true.
      _trace_service->trin_propagate_area(
        area_name              = area_name
        client                 = l_client
        affecting_local_server = affecting_local_server
      ).
    ENDIF.
  ENDIF.
...
* <
  ...
  IF _trace_active = abap_true.
    IF _trace_service->variant-propagate_inst = abap_true.
      _trace_service->trin_propagate_instance(
      area_name         = area_name
      inst_name         = inst_name
      client            = l_client
      affecting_local_server = affecting_local_server
